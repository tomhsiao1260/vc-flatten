...
def gen_sub_uv(x_max, y_max, z_max):
    ...
    for i, contour in enumerate(contours_left):
        ...
        mask = np.zeros_like(binary_img_left).astype(np.uint16)
        cv2.drawContours(mask, [contour], -1, 65535, thickness=cv2.FILLED)
        uv_map_left[:, :, 3] = mask
        [x, y, w, h] = cv2.boundingRect(contour)
        cv2.imwrite(uv_name, uv_map_left[:, x:(x+w), :])

        gen_sub_d(uv_name, d_name, position_map)
        # if (mask_count == 2): gen_sub_d(uv_name, d_name, position_map)
    ...
    for i, contour in enumerate(contours_right):
        ...
        mask = np.zeros_like(binary_img_right).astype(np.uint16)
        cv2.drawContours(mask, [contour], -1, 65535, thickness=cv2.FILLED)
        uv_map_right[:, :, 3] = mask
        [x, y, w, h] = cv2.boundingRect(contour)
        cv2.imwrite(uv_name, uv_map_right[:, x:(x+w), :])
